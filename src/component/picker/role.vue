<template lang="pug">
el-drawer(title="选择角色", :model-value="modelValue", size="900px", center, @close="handleClose")
  ...
  el-table(:data="list", border, highlight-current-row, ref="tableRef", @current-change="handlePickChange")
    //- el-table-column(fixed="left", width="60px", align="center")
      template(#default="{ row }")
        el-icon(size="20", color="white")
          CircleCheck 
    el-table-column(label="名称", prop="name")
    el-table-column(label="备注", prop="remark")
    el-table-column(label="成员", prop="accountCount")
      template(#default="{ row }")
        el-tag(type="success") {{ row.accountCount }}
