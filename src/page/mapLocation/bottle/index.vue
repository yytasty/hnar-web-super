  ...
  el-card
    el-form(inline)
      ...
      el-form-item(label="钢瓶状态")
        el-select(v-model="filter.status", @change="handleSearch")
          el-option(label="全部", value="")
  ...
  .result
    el-card(header="结果列表", style="flex-basis: 400px")
      el-table(:data="list")
        ...
        el-table-column(label="钢瓶条码", prop="sn")
        el-table-column(label="钢瓶规格", prop="bottleSpecific.name")
        el-table-column(label="地址定位")
          template(#default="{ row }")
            el-tooltip(
              v-if="row.location?.coordinates?.[0]",
              :content="row.location?.coordinates.toString()",
              placement="top"
            )
...
function refreshMap() {
  ...
  list.value.forEach((i, n) => {
    if (i.location?.coordinates?.[0]) {
      console.log(">>>>>>> n", n)
      ...
      marker.on("click", () => {
        mapInstance.value.setCenter(i.location?.coordinates, true)
      })
...
function handleMoveMap(row) {
  if (row.location?.coordinates?.[0]) {
    mapInstance.value.setCenter(row.location?.coordinates, true)
  }
}
